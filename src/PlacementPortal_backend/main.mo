...
actor JobPortal {
    ...
    var candidateMap = Map.fromIter<Text, [Text]>(
        candidateApplications.vals(),
        10,
        Text.equal,
        Text.hash,
    );

    var companyMap = Map.fromIter<Text, Nat>(
        companyArray.vals(),
        10,
        Text.equal,
        Text.hash,
    ...
    var shortListMap = Map.fromIter<Text, [Text]>(
        shortLists.vals(),
        10,
        Text.equal,
        Text.hash,
    ...
    var globalShortlistMap = Map.fromIter<Text, Text>(
        globalShortlist.vals(),
        10,
        Text.equal,
        Text.hash,
    );
                    ...
                    case (?companyId) {
                        switch (candidateMap.get(candidateName)) {
                            ...
                            case (?appliedCompanies) {
                                let alreadyApplied = Array.find<Text>(
                                    appliedCompanies,
                                    func(company) {
                                        company == companyName;
                                    },
        ...
        switch (companyMap.get(companyName)) {
            ...
            case (?companyId) {
                ...
                switch (candidateMap.get(candidateName)) {
                    ...
                    case (?appliedCompanies) {
                        let appliedToThisCompany = Array.find<Text>(
                            appliedCompanies,
                            func(company) {
                                company == companyName;
                            },
    ...
    // Clear registered companies
    companyArray := [];

    // Clear global shortlist mapping
    globalShortlist := [];

    // Reinitialize maps to clear their contents
    candidateMap := Map.HashMap<Text, [Text]>(10, Text.equal, Text.hash);
    companyMap := Map.HashMap<Text, Nat>(10, Text.equal, Text.hash);
    shortListMap := Map.HashMap<Text, [Text]>(10, Text.equal, Text.hash);
    globalShortlistMap := Map.HashMap<Text, Text>(10, Text.equal, Text.hash);
};
